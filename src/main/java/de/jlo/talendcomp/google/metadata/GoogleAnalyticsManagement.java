...
public class GoogleAnalyticsManagement {
	...
	private Credential authorizeWithServiceAccount() throws Exception {
		...
		if (keyFile.canRead() == false) {
			throw new IOException("Key file:" + keyFile.getAbsolutePath()
					+ " is not readable");
		}
		if (keyFile.getName().toLowerCase().endsWith(".json")) {
			GoogleCredential credential = GoogleCredential.fromStream(new FileInputStream(keyFile))
				    .createScoped(Arrays.asList(AnalyticsScopes.ANALYTICS_READONLY, AnalyticsScopes.ANALYTICS_MANAGE_USERS));
			return credential;
